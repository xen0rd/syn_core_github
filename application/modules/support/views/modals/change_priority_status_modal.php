<div class="modal-header <?=$userInfo->theme_name?>">
	...
	<h4 class="modal-title">Change Priority Status</h4>
</div>
<div class="modal-body">
	<p>Are you sure you want to change this priority level's status?</p>
</div>
...
<script>
	$("#confirm_status_change").click(function(){
		$.ajax({
			url:"<?=site_url('support/changePriorityStatus');?>",
			type: "POST",
			...
			success: function(data){
				if(data === "TRUE"){
					$('#toggle_priority_' + <?=$this->uri->segment(3); ?>).bootstrapToggle("toggle");
